...
export default ({ publicStore = false }) => {
  function populateRow(row, metadata = {}) {
    row._ = {
      metadata,
      loading: false,
      hover: false,
  ...
  const mutations = {
    ...
    DECREASE_ORDERS_IN_BUFFER_LOWER_THAN(state, existingOrder) {
      const min = new BigNumber(existingOrder).integerValue(
    ...
    INSERT_NEW_ROW_IN_BUFFER_AT_INDEX(state, { row, index }) {
      ...
      if (
        state.rows.findIndex((r) => r.id !== row.id && r.order === row.order) >
        -1
      ) {
