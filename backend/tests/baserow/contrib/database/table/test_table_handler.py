...
@patch("baserow.contrib.database.table.handler.JobHandler.create_and_start_job")
def test_fill_table_with_initial_data(mock_create_job, data_fixture):
    user = data_fixture.create_user()
    database = data_fixture.create_database_application(user=user)
    ...
    mock_create_job.assert_called_once_with(user, "file_import", data=data, table=table)
