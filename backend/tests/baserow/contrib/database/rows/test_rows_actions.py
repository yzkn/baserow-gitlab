...
def test_can_undo_deleting_row(data_fixture):
    ...
    action_type_registry.get_by_type(DeleteRowActionType).do(
        user=user, table=table, row_id=row.id
    )
    assert model.objects.all().count() == 0
...
def test_can_undo_moving_row(data_fixture):
    ...
    action_type_registry.get_by_type(MoveRowActionType).do(
        user=user, table=table, row_id=row_1.id
    )
...
@pytest.mark.django_db
def test_can_undo_updating_row(data_fixture):
    session_id = "session-id"
    user = data_fixture.create_user(session_id=session_id)
    table = data_fixture.create_database_table(name="Car", user=user)
    name_field = data_fixture.create_text_field(
    ...
    action_type_registry.get_by_type(UpdateRowActionType).do(
        ...
        row_id=row.id,
        values={
            name_field.id: "Amazon",
            speed_field.id: 12,
            f"field_{price_field.id}": 19.99,
        },
    )
    row.refresh_from_db()
    assert getattr(row, f"field_{name_field.id}") == "Amazon"
    assert getattr(row, f"field_{speed_field.id}") == 12
    assert getattr(row, f"field_{price_field.id}") == Decimal("19.99")
